...
namespace ctl {
    ...
    template<class T>
    std::string join(const std::string& sep, const std::vector<T>& v) {
        std::string s;
...
#define CHECK(x) { if (!(x)) throw std::runtime_error(std::format("\"{}\" Failed\n", #x));}
    ...
    struct intmap {
        std::vector<int> map;
        vec2 size;
        void init(vec2 res, int i) {
            map.resize(res.x * res.y, i);
            size = res;
        }
        bool isinside(vec2 p) const {
            return p.x < size.x && p.y < size.y && p.x >= 0 && p.y >= 0;
        }
        int& get(vec2 p) {
            if (isinside(p)) {
                return map[p.y * size.x + p.x];
            }
            throw std::runtime_error(std::format("intmap get OOB ({}, {})\n", p.x, p.y));
        }
    };

    struct map_t {
        std::vector<std::string> map;
        void init(vec2 res, char c) {
            for (int y = 0; y < res.y; ++y) {
                map.push_back(std::string(res.x, c));
            }
        }
